...
import torch.nn
from torch.nn import CrossEntropyLoss, MSELoss,TransformerDecoder
...
class NetfoundDecoderModel(NetFoundPretrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        # detached to keep the pretrained model unchanged
        sequence_output = sequence_output.detach() #keeping the encoder unchanged 
        # pooled_output = poolingByAttention(
        #     self.attentivePooling, sequence_output, self.config.max_burst_length
        # )
        # burst_attention = attention_mask[:, ::self.max_burst_length].detach().clone()
        ...
        #decoder code, pooled output is the flow representation
        ...
        decoder_outputs=self.decoder(embeddings,sequence_output) #getting decoder outputs
        prediction_scores = self.lm_head(decoder_outputs)
        ...
        if not return_dict:
            print(return_dict)
        #final_output=decoder_outputs[0]
