...
class DataCollatorWithMeta(DataCollatorForLanguageModeling):
    ...
    def torch_call(
        ...
        for i in range(len(examples)):
            ...
            for key in inputs.keys():
                ...
                #         batch["replaced"] = []
                if key == "labels" or key == "totalBursts" or key == "replacedAfter":
                    continue
                if key not in batch:
                    if key != "replacedAfter":
                        batch[key] = []
    ...
    def torch_mask_tokens(self, input_ids, protos, **kwargs):
        ...
        # print("input_ids before masking:", input_ids)
        ...
        # )
        #probability_matrix = self.maskBasedOnPos(labels, protos, 5, protoNeeded=6)
