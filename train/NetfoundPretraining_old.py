...
def main():
    ...
    if model_args.model_name_or_path is not None and os.path.exists(
        ...
        get_model = lambda: NetFoundLanguageModelling.from_pretrained(
    ...
    def compute_metrics(eval_preds):
        ...
        return_metrics = {
            "mlm_f1": f1_score(labels, preds, average="macro", zero_division=1),
            "mlm_prec": precision_score(labels, preds, average="macro", zero_division=1),
            "mlm_recall": recall_score(labels, preds, average="macro", zero_division=1),
            "weighted_mlm_f1": f1_score(labels, preds, average="weighted", zero_division=1),
            "weighted_mlm_prec": precision_score(labels, preds, average="weighted", zero_division=1),
            "weighted_mlm_recall": recall_score(labels, preds, average="weighted", zero_division=1),
